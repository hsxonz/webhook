...
const CONNECT_FAILURE_REASONS = {
  400: 'lỗi chung',
  401: 'đã logout khỏi thiết bị (thường do người dùng tự gỡ liên kết)',
  402: 'bị cấm tạm thời',
  403: 'máy chính bị gỡ liên kết — WhatsApp Web gọi là LOCKED, có thể do ban hoặc do đổi điện thoại',
...
// 403 và 406 là mất tài khoản, không tự phục hồi. 401 chỉ là gỡ liên kết bình thường.
 ...
 */
export const extract = (event, payload) => {
  const at = moment().tz(config.timezone).format('YYYY-MM-DD HH:mm:ss');

  if (event === 'session.status') {
    return { at, kind: 'status', severity: 'info', status: payload?.status ?? null };
  }
  ...
  const base = { at, kind, severity: SEVERITY[kind] ?? 'info' };
...
export const sweepDisconnected = async () => {
  try {
    ...
    const cutoff = Date.now() - ALERT_AFTER_SEC * 1000;
    for (const [kind, cfg] of Object.entries(PENDING)) {
      const stale = await redis.client.zRangeByScore(cfg.key, 0, cutoff);
      const giveUp = Date.now() - GIVE_UP_AFTER_SEC * 1000;
      for (const session of stale) {
        const age = await redis.client.zScore(cfg.key, session);
        if (age && age < giveUp) {
          await redis.client.zRem(cfg.key, session);
        ...
        const sent = await telegram.sendOnce(
          ...
          [
            ...
            `${cfg.detail} Quá 5 phút là bất thường.`,
          ].join('\n'),
        );
...
const alert = async (session, entry) => {
  ...
  const lines = [title, '', `Session: <b>${session}</b>`, `Loại: <b>${entry.kind}</b>`];
