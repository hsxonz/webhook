...
const CONNECT_FAILURE_REASONS = {
  ...
  402: 'bị cấm tạm thời',
  403: 'máy chính đăng xuất hoặc đổi điện thoại — quét QR lại là dùng tiếp',
...
const SILENT_KINDS = new Set([
  ...
  'KeepAliveTimeout',
  'KeepAliveRestored',
  // Đã vào sổ chờ ở PENDING, để sweeper quyết định có báo hay không.
  'StreamReplaced',
]);
...
// Chỉ 406 mới thật sự là mất tài khoản. whatsmeow ghi rõ 403 lưỡng nghĩa —
// "seems to happen for both bans and switching phones" (events.go:216) — và thực
// tế phần lớn là đổi máy hoặc người dùng tự đăng xuất, quét QR lại là dùng tiếp.
// Gộp nó vào nhóm vĩnh viễn thì mọi lần gỡ liên kết bình thường đều báo động giả.
const PERMANENT_CODES = new Set([406]);
// 401 và 403 mất phiên nhưng số vẫn còn: chỉ cần quét QR lại.
const RELINK_CODES = new Set([401, 403]);
